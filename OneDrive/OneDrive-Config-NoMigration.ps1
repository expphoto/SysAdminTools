...
$logFile = Join-Path $logDir ("OneDriveConfig_{0}.log" -f $env:COMPUTERNAME)
...
function Test-HiveLoaded {
    ...
    Test-Path ("Registry::HKEY_USERS\{0}" -f $Sid)
}
...
function Invoke-PerUserRegistryWork {
    param(
    ...
    Log ("Applying per-user configuration for {0} ({1})" -f $ProfilePath, $Sid)
    ...
    foreach ($entry in $folderMap.GetEnumerator()) {
        ...
        $expandedPath = ('%USERPROFILE%\{0}' -f $entry.Value)

        Ensure-RegistryValue -Path $userShellFolders -Name $entry.Key -Value $expandedPath -Type ExpandString
        Ensure-RegistryValue -Path $shellFolders -Name $entry.Key -Value $resolvedPath -Type String
        Log ("Reset {0} -> {1}" -f $entry.Key, $resolvedPath)
    }

    $accountsRoot = Join-Path $RegistryRoot 'SOFTWARE\Microsoft\OneDrive\Accounts'
    if (Test-Path $accountsRoot) {
        Get-ChildItem -Path $accountsRoot -ErrorAction SilentlyContinue | ForEach-Object {
            ...
            Log ("Cleared KFM flags under {0}" -f $_.PSChildName)
        }
    }
...
function Mount-UserHive {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to load hive {0} from {1}" -f $Sid, $NtUserDat
    }
}
...
function Get-ActiveSessions {
    ...
    try {
        ...
        foreach ($line in $lines | Select-Object -Skip 1) {
            ...
            if ($parts[2] -eq 'Active') {
                $sessions += $parts[0]
            }
        }
    } catch {
        Log 'INFO: Could not enumerate active sessions via quser'
    }

    $sessions | Select-Object -Unique
}

function Restart-ProcessesForActiveUsers {
    $activeUsers = Get-ActiveSessions
    if (-not $activeUsers) {
        Log 'INFO: No active sessions found for Explorer/OneDrive restart'
        return
    }
...
'@
        ...
        try {
            $action = New-ScheduledTaskAction -Execute 'powershell.exe' -Argument ("-NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -Command {0}" -f ([Management.Automation.Language.CodeGeneration]::QuoteArgument($command)))
            ...
            Log ("Triggered Explorer/OneDrive restart for active user {0}" -f $username)
            Start-Sleep -Seconds 20
        } catch {
            Log ("WARNING: Failed to restart Explorer/OneDrive for {0}: {1}" -f $username, $_.Exception.Message)
        } finally {
...
Log ("HKLM OneDrive policy written for tenant {0}" -f $TenantID)
...
foreach ($profile in $profiles) {
    ...
    try {
        ...
        Invoke-PerUserRegistryWork -RegistryRoot ("Registry::HKEY_USERS\{0}" -f $profile.Sid) -ProfilePath $profile.ProfilePath -Sid $profile.Sid
    } catch {
